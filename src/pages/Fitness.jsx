...
export default function Fitness({ currentPlayer, isAdmin }) {
  ...
  const getRanked = () => {
    return groupPlayers.map(p => {
      const playerTests = tests
        .filter(t => t.player_id === p.id && t.test_type === activeTest)
  ...
  const notTested = groupPlayers.filter(p =>
    !tests.some(t => t.player_id === p.id && t.test_type === activeTest)
  )
            ...
            <tbody>
              {ranked.map(({ p, last, prev }, i) => {
                ...
                if (activeTest === 'cooper') {
                  ...
                  const evol = diff !== null
                    ? diff >= 0
                      ? <span className="evol-up">▲ +{diff}m</span>
                      : <span className="evol-down">▼ {diff}m</span>
                    : '—'
                  return (
                    <tr key={p.id}>
                      <td><span className={`rank-num ${rc}`}>{i + 1}</span></td>
                      <td style={{ fontWeight: 500, cursor: 'pointer', color: 'var(--red)' }} onClick={() => setSelected(p)}>
                        {p.first_name} {p.last_name}
                      </td>
                      <td>{last.value}m</td>
                      <td><span className={`pill pill-${lv.c}`}>{vo2.toFixed(1)} — {lv.l}</span></td>
                      <td>{evol}</td>
                      <td style={{ color: 'var(--gray-4)', fontSize: 11 }}>{last.test_date}</td>
                    </tr>
                  )
                } else {
                  const lv = sprintLevel(last.value)
                  const diff = prev ? last.value - prev.value : null
                  const evol = diff !== null
                    ? diff <= 0
                      ? <span className="evol-up">▲ {diff.toFixed(2)}s</span>
                      : <span className="evol-down">▼ +{diff.toFixed(2)}s</span>
                    : '—'
                  return (
                    <tr key={p.id}>
                      <td><span className={`rank-num ${rc}`}>{i + 1}</span></td>
